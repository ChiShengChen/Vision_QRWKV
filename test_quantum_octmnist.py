...
def run_quantum_octmnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for quantum OCTMNIST classification test") # Updated print
    ...
    print(f"Quantum Model Config for OCTMNIST: {config}\n")

    model = QuantumRWKVModel(config)
    model.to(device)
    print(f"Quantum RWKVModel for OCTMNIST instantiated successfully.\n")
    ...
    print(f"Starting quantum training for OCTMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Quantum OCTMNIST)')
    ...
    print(f"Quantum OCTMNIST training finished.\n")
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Quantum OCTMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='purple'); plt.title('Test Acc (Quantum OCTMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Quantum OCTMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_quantum_octmnist_classification() 