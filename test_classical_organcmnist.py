...
from medmnist import OrganCMNIST # Changed for OrganCMNIST
...
def run_classical_organcmnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical OrganCMNIST classification test") # Updated print
    ...
    dataset_name_lower = "organcmnist"
    input_channels = 1 
    num_classes = 11 # 11 classes for OrganCMNIST
    mean_val = (0.2981,)
    std_val = (0.2642,)
    ...
    # Use as_rgb=False for OrganCMNIST as per MedMNIST documentation
    train_dataset = OrganCMNIST(split='train', transform=transform, download=True, root='./data', as_rgb=False)
    test_dataset = OrganCMNIST(split='test', transform=transform, download=True, root='./data', as_rgb=False)
    ...
    print(f"Classical Model Config for OrganCMNIST: {config}\n")

    model = RWKVModel(config)
    model.to(device)
    print(f"Classical RWKVModel for OrganCMNIST instantiated successfully.\n")
    ...
    print(f"Starting classical training for OrganCMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Classical OrganCMNIST)')
    ...
    print(f"Classical OrganCMNIST training finished.\n")
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Classical OrganCMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='orange'); plt.title('Test Acc (Classical OrganCMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Classical OrganCMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_classical_organcmnist_classification() 