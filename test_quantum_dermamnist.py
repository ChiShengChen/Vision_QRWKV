...
def run_quantum_dermamnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for quantum DermaMNIST classification test") # Updated print
    ...
    print(f"Quantum Model Config for DermaMNIST: {config}\n")

    model = QuantumRWKVModel(config)
    model.to(device)
    print(f"Quantum RWKVModel for DermaMNIST instantiated successfully.\n")
    ...
    print(f"Starting quantum training for DermaMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Quantum DermaMNIST)')
    ...
    print(f"Quantum DermaMNIST training finished.\n")
    ...
    header = ['Timestamp', 'Experiment_ID', 'Model_Type', 'Task', 'n_layer', 'n_embd', 'n_head', 'n_qubits', 'q_depth',
    ...
    data_row = [training_start_time_obj.strftime("%Y-%m-%d %H:%M:%S"), experiment_id, 'Quantum', f'{dataset_name_lower.capitalize()}MNIST_8x8',
                config.n_layer, config.n_embd, config.n_head, config.n_qubits, config.q_depth, 
                optimizer.param_groups[0]['lr'], num_epochs, seq_len, input_channels, num_classes, 
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Quantum DermaMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='purple'); plt.title('Test Acc (Quantum DermaMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Quantum DermaMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_quantum_dermamnist_classification() 