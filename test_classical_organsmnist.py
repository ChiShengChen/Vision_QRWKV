...
from medmnist import OrganSMNIST # Changed for OrganSMNIST
...
def run_classical_organsmnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical OrganSMNIST classification test") # Updated print
    ...
    dataset_name_lower = "organsmnist"
    input_channels = 1 
    num_classes = 11 # 11 classes for OrganSMNIST
    mean_val = (0.2396,)
    std_val = (0.2610,)
    ...
    # Use as_rgb=False for OrganSMNIST as per MedMNIST documentation
    train_dataset = OrganSMNIST(split='train', transform=transform, download=True, root='./data', as_rgb=False)
    test_dataset = OrganSMNIST(split='test', transform=transform, download=True, root='./data', as_rgb=False)
    ...
    print(f"Classical Model Config for OrganSMNIST: {config}\n")

    model = RWKVModel(config)
    model.to(device)
    print(f"Classical RWKVModel for OrganSMNIST instantiated successfully.\n")
    ...
    print(f"Starting classical training for OrganSMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Classical OrganSMNIST)')
    ...
    print(f"Classical OrganSMNIST training finished.\n")
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Classical OrganSMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='orange'); plt.title('Test Acc (Classical OrganSMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Classical OrganSMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_classical_organsmnist_classification() 