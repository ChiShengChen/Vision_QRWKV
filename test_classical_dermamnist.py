...
from medmnist import DermaMNIST # Changed for DermaMNIST
...
def run_classical_dermamnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical DermaMNIST classification test") # Updated print
    ...
    dataset_name_lower = "dermamnist"
    input_channels = 3 
    num_classes = 7
    mean_val = (0.7630, 0.5435, 0.5704)
    std_val = (0.1407, 0.1531, 0.1682)
    ...
    train_dataset = DermaMNIST(split='train', transform=transform, download=True, as_rgb=True, root='./data')
    test_dataset = DermaMNIST(split='test', transform=transform, download=True, as_rgb=True, root='./data')
    ...
    print(f"Classical Model Config for DermaMNIST: {config}\n")

    model = RWKVModel(config)
    model.to(device)
    print(f"Classical RWKVModel for DermaMNIST instantiated successfully.\n")
    ...
    print(f"Starting classical training for DermaMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.title(f'CM Epoch {epoch+1} (Classical DermaMNIST)')
    ...
    print(f"Classical DermaMNIST training finished.\n")
    ...
    header = ['Timestamp', 'Experiment_ID', 'Model_Type', 'Task', 'n_layer', 'n_embd', 'n_head', 
              'lr', 'epochs', 'seq_len', 'input_channels', 'num_classes', 'Accuracy', 'Loss']
    experiment_id = f"c_{dataset_name_lower}_{training_start_time_str}"
    data_row = [training_start_time_obj.strftime("%Y-%m-%d %H:%M:%S"), experiment_id, 'Classical', f'{dataset_name_lower.capitalize()}MNIST_8x8',
                config.n_layer, config.n_embd, config.n_head, optimizer.param_groups[0]['lr'], num_epochs, 
                seq_len, input_channels, num_classes, f'{final_accuracy:.2f}', f'{final_train_loss:.6f}']
    try:
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Classical DermaMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='orange'); plt.title('Test Acc (Classical DermaMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Classical DermaMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_classical_dermamnist_classification() 