...
from medmnist import OCTMNIST # Changed for OCTMNIST
...
def run_classical_octmnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical OCTMNIST classification test") # Updated print
    ...
    train_dataset = OCTMNIST(split='train', transform=transform, download=True, root='./data')
    test_dataset = OCTMNIST(split='test', transform=transform, download=True, root='./data')
    ...
    print(f"Classical Model Config for OCTMNIST: {config}\n")

    model = RWKVModel(config)
    model.to(device)
    print(f"Classical RWKVModel for OCTMNIST instantiated successfully.\n")
    ...
    print(f"Starting classical training for OCTMNIST classification...")
    for epoch in range(num_epochs):
        model.train()
        epoch_loss_sum = 0.0
        num_batches_processed = 0
        for batch_idx, (data, target) in enumerate(train_loader):
            data = data.view(data.size(0), -1, input_channels).to(device) # For grayscale
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.figure(figsize=(8, 6)) # Adjusted size for fewer classes
            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=range(num_classes), yticklabels=range(num_classes))
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Classical OCTMNIST)')
            cm_plot_filename = os.path.join(results_dir, f"cm_classical_{dataset_name_lower}_epoch{epoch+1}_{training_start_time_str}.png")
            try: plt.savefig(cm_plot_filename); print(f"CM plot saved: {cm_plot_filename}"); plt.close()
            except Exception as e: print(f"Error saving CM plot: {e}")

    print(f"Classical OCTMNIST training finished.\n")
    ...
    overall_metrics_csv_filename = os.path.join(results_dir, f"summary_classical_{dataset_name_lower}.csv")
    # Using the more detailed header from previous files for consistency
    header = [
    ...
    data_row = [training_start_time_obj.strftime("%Y-%m-%d_%H-%M-%S"), experiment_id, 'Classical', f'{dataset_name_lower.capitalize()}MNIST_8x8',
                config.n_layer, config.n_embd, config.n_head, 'N/A', 'N/A',
                optimizer.param_groups[0]['lr'], num_epochs, 
                seq_len, input_channels, config.block_size, config.n_intermediate, num_classes, 
                f'{final_accuracy:.2f}', f'{final_train_loss:.6f}']
    # Correcting the order for the detailed header
    data_row = [
        training_start_time_obj.strftime("%Y-%m-%d_%H-%M-%S"), experiment_id, 'Classical', f'{dataset_name_lower.capitalize()}MNIST_8x8',
        config.n_layer, config.n_embd, config.n_head, 'N/A', 'N/A',
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Classical OCTMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='orange'); plt.title('Test Acc (Classical OCTMNIST)'); plt.tight_layout()
    plot_fn = os.path.join(results_dir, f"progress_classical_{dataset_name_lower}_{training_start_time_str}.png")
    ...
    print(f"\n=== Finished Classical OCTMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_classical_octmnist_classification() 