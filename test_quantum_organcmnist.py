...
def run_quantum_organcmnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for quantum OrganCMNIST classification test") # Updated print
    ...
    print(f"Quantum Model Config for OrganCMNIST: {config}\n")

    model = QuantumRWKVModel(config)
    model.to(device)
    print(f"Quantum RWKVModel for OrganCMNIST instantiated successfully.\n")
    ...
    print(f"Starting quantum training for OrganCMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Quantum OrganCMNIST)')
    ...
    print(f"Quantum OrganCMNIST training finished.\n")
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Quantum OrganCMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='purple'); plt.title('Test Acc (Quantum OrganCMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Quantum OrganCMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_quantum_organcmnist_classification() 