...
def run_classical_fashion_mnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Updated print statement for Fashion-MNIST
    print(f"Using device: {device} for classical Fashion-MNIST classification test")
    ...
    base_results_dir = "results_fashion_mnist_classical"
    ...
    # Model Configuration (remains the same as per request)
    ...
    print(f"Classical Model Config for Fashion-MNIST: {config}\n")
    ...
    print("Classical RWKVModel for Fashion-MNIST instantiated successfully.\n")
    ...
    epoch_metrics_csv_filename = os.path.join(results_dir, "epoch_metrics_classical_fashion.csv")
    ...
    # Updated print statement
    print("Starting classical training for Fashion-MNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            # Updated plot title
            plt.title(f'Confusion Matrix - Epoch {epoch+1} (Classical Fashion-MNIST)')
            # Updated plot filename
            cm_plot_filename = os.path.join(results_dir, f"confusion_matrix_classical_fashion_epoch{epoch+1}_{training_start_time_str}.png")
            try:
    ...
    # Updated print statement
    print("Classical Fashion-MNIST training finished.\n")

    final_accuracy = all_epoch_test_accuracies[-1]
    final_train_loss = all_epoch_train_losses[-1]

    # Updated summary CSV filename
    overall_metrics_csv_filename = os.path.join(results_dir, "model_performance_summary_classical_fashion.csv")
    header = [
    ...
    experiment_id = f"c_fashion_mnist_{training_start_time_str}"
    learning_rate = optimizer.param_groups[0]['lr']

    data_row = [
        training_start_time_obj.strftime("%Y-%m-%d_%H-%M-%S"),
        experiment_id, 'Classical', 'FashionMNIST_8x8', # Updated Task name
    ...
    plt.ylabel('Loss')
    # Updated plot title
    plt.title('Training Loss Over Epochs (Classical Fashion-MNIST)')
    plt.legend()
    plt.grid(True)
    ...
    plt.ylabel('Accuracy (%)')
    # Updated plot title
    plt.title('Test Accuracy Over Epochs (Classical Fashion-MNIST)')
    plt.legend()
    plt.grid(True)

    plt.tight_layout()
    # Updated plot filename
    epoch_plot_filename = os.path.join(results_dir, f"training_progress_classical_fashion_{training_start_time_str}.png")
    try:
    ...
    print("\n=== Finished Classical Fashion-MNIST Classification Test ===\n")

if __name__ == '__main__':
    # Updated function call
    run_classical_fashion_mnist_classification() 