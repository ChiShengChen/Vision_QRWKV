...
def run_classical_tissuemnist_classification():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Updated print statement for TissueMNIST
    print(f"Using device: {device} for classical TissueMNIST classification test")
    ...
    print(f"Classical Model Config for TissueMNIST: {config}\n")
    ...
    try:
        ...
        raise
    print(f"Classical RWKVModel for TissueMNIST instantiated successfully.\n")
    ...
    print(f"Starting classical training for TissueMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.title(f'Confusion Matrix - Epoch {epoch+1} (Classical TissueMNIST)')
    ...
    print(f"Classical TissueMNIST training finished.\n")
    ...
    data_row = [
        ...
        experiment_id, 'Classical', f'TissueMNIST_8x8', 
    ...
    if all_epoch_train_losses:
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title('Training Loss Over Epochs (Classical TissueMNIST)')
        plt.legend()
        plt.grid(True)
    ...
    if all_epoch_test_accuracies:
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy (%)')
        plt.title('Test Accuracy Over Epochs (Classical TissueMNIST)')
        plt.legend()
        plt.grid(True)
    ...
    print(f"\n=== Finished Classical TissueMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_classical_tissuemnist_classification() 