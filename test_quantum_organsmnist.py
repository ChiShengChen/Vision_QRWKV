...
def run_quantum_organsmnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for quantum OrganSMNIST classification test") # Updated print
    ...
    print(f"Quantum Model Config for OrganSMNIST: {config}\n")

    model = QuantumRWKVModel(config)
    model.to(device)
    print(f"Quantum RWKVModel for OrganSMNIST instantiated successfully.\n")
    ...
    print(f"Starting quantum training for OrganSMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Quantum OrganSMNIST)')
    ...
    print(f"Quantum OrganSMNIST training finished.\n")
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Quantum OrganSMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='purple'); plt.title('Test Acc (Quantum OrganSMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Quantum OrganSMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_quantum_organsmnist_classification() 