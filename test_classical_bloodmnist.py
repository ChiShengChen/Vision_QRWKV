...
# MedMNIST import
from medmnist import BloodMNIST # Changed from RetinaMNIST
...
# Function name changed for BloodMNIST
def run_classical_bloodmnist_classification():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Updated print statement for BloodMNIST
    print(f"Using device: {device} for classical BloodMNIST classification test")

    training_start_time_obj = datetime.datetime.now()
    training_start_time_str = training_start_time_obj.strftime("%Y%m%d_%H%M%S")
    
    # BloodMNIST Data loading and preprocessing
    image_size = 8
    batch_size_train = 64
    batch_size_test = 1000
    
    # Specifics for BloodMNIST
    dataset_name_lower = "bloodmnist"
    input_channels = 3 
    num_classes = 8 # 8 classes for BloodMNIST
    # Normalization for BloodMNIST (RGB)
    mean_val = (0.7327, 0.5633, 0.7016)
    std_val = (0.1937, 0.2087, 0.1648)
    ...
    # Updated base directory name for BloodMNIST
    ...
    # BloodMNIST DataLoaders
    # Using as_rgb=True for consistency, though BloodMNIST is already RGB
    train_dataset = BloodMNIST(split='train', transform=transform, download=True, as_rgb=True, root='./data')
    test_dataset = BloodMNIST(split='test', transform=transform, download=True, as_rgb=True, root='./data')
    ...
    output_dim_test = num_classes # 8 classes for BloodMNIST
    ...
    # Updated print statement for BloodMNIST config
    print(f"Classical Model Config for BloodMNIST: {config}\n")
    ...
    print(f"Classical RWKVModel for BloodMNIST instantiated successfully.\n")
    ...
    # Updated print statement
    print(f"Starting classical training for BloodMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.figure(figsize=(10, 8)) # Suitable for 8 classes
            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                        xticklabels=range(num_classes), yticklabels=range(num_classes))
            plt.xlabel('Predicted Label')
            plt.ylabel('True Label')
            plt.title(f'Confusion Matrix - Epoch {epoch+1} (Classical BloodMNIST)')
    ...
    print(f"Classical BloodMNIST training finished.\n")
    ...
    data_row = [
        ...
        experiment_id, 'Classical', f'BloodMNIST_8x8', 
    ...
    if all_epoch_train_losses:
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title('Training Loss Over Epochs (Classical BloodMNIST)')
        plt.legend()
        plt.grid(True)
    ...
    if all_epoch_test_accuracies:
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy (%)')
        plt.title('Test Accuracy Over Epochs (Classical BloodMNIST)')
        plt.legend()
        plt.grid(True)
    ...
    print(f"\n=== Finished Classical BloodMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_classical_bloodmnist_classification() 