...
from medmnist import PneumoniaMNIST # Changed for PneumoniaMNIST
...
def run_classical_pneumoniamnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical PneumoniaMNIST classification test") # Updated print
    ...
    dataset_name_lower = "pneumoniamnist"
    input_channels = 1 
    ...
    train_dataset = PneumoniaMNIST(split='train', transform=transform, download=True, root='./data')
    test_dataset = PneumoniaMNIST(split='test', transform=transform, download=True, root='./data')
    ...
    print(f"Classical Model Config for PneumoniaMNIST: {config}\n")

    model = RWKVModel(config)
    model.to(device)
    print(f"Classical RWKVModel for PneumoniaMNIST instantiated successfully.\n")
    ...
    print(f"Starting classical training for PneumoniaMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Classical PneumoniaMNIST)')
    ...
    print(f"Classical PneumoniaMNIST training finished.\n")
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Classical PneumoniaMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='orange'); plt.title('Test Acc (Classical PneumoniaMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Classical PneumoniaMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_classical_pneumoniamnist_classification() 