...
# MedMNIST import
from medmnist import PathMNIST # Changed from torchvision.datasets
...
def run_classical_pathmnist_classification():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Updated print statement for PathMNIST
    print(f"Using device: {device} for classical PathMNIST classification test")
    ...
    # Using as_rgb=True for consistency, though PathMNIST is already RGB
    ...
    print(f"Classical Model Config for PathMNIST: {config}\n")
    ...
    print(f"Classical RWKVModel for PathMNIST instantiated successfully.\n")
    ...
    # Updated print statement
    print(f"Starting classical training for PathMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            # Use a different cmap if desired, e.g. 'viridis'
            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                        xticklabels=range(num_classes), yticklabels=range(num_classes))
            plt.xlabel('Predicted Label')
            plt.ylabel('True Label')
            # Updated plot title
            plt.title(f'Confusion Matrix - Epoch {epoch+1} (Classical PathMNIST)')
    ...
    # Updated print statement
    print(f"Classical PathMNIST training finished.\n")
    ...
    data_row = [
        ...
        experiment_id, 'Classical', f'PathMNIST_8x8', # Updated Task name
    ...
    if all_epoch_train_losses:
        ...
        plt.ylabel('Loss')
        # Updated plot title
        plt.title('Training Loss Over Epochs (Classical PathMNIST)')
        plt.legend()
        plt.grid(True)
    ...
    if all_epoch_test_accuracies:
        ...
        plt.ylabel('Accuracy (%)')
        # Updated plot title
        plt.title('Test Accuracy Over Epochs (Classical PathMNIST)')
        plt.legend()
        plt.grid(True)
    ...
    print(f"\n=== Finished Classical PathMNIST Classification Test ===\n")

if __name__ == '__main__':
    # Updated function call
    run_classical_pathmnist_classification() 