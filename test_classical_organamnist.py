...
from medmnist import OrganAMNIST # Changed for OrganAMNIST
...
def run_classical_organamnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical OrganAMNIST classification test") # Updated print
    ...
    dataset_name_lower = "organamnist"
    input_channels = 1 
    num_classes = 11 # 11 classes for OrganAMNIST
    mean_val = (0.2606,)
    std_val = (0.2538,)
    ...
    # Use as_rgb=False for OrganAMNIST as per MedMNIST documentation
    train_dataset = OrganAMNIST(split='train', transform=transform, download=True, root='./data', as_rgb=False)
    test_dataset = OrganAMNIST(split='test', transform=transform, download=True, root='./data', as_rgb=False)
    ...
    print(f"Classical Model Config for OrganAMNIST: {config}\n")

    model = RWKVModel(config)
    model.to(device)
    print(f"Classical RWKVModel for OrganAMNIST instantiated successfully.\n")
    ...
    print(f"Starting classical training for OrganAMNIST classification...")
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for data, target in test_loader:
                ...
                target_squeezed = target.view(-1).long().to(device)
                predictions, _ = model(data, states=None)
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.figure(figsize=(12, 10)) # Adjusted size for 11 classes
            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=range(num_classes), yticklabels=range(num_classes))
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Classical OrganAMNIST)')
    ...
    print(f"Classical OrganAMNIST training finished.\n")
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Classical OrganAMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='orange'); plt.title('Test Acc (Classical OrganAMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Classical OrganAMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_classical_organamnist_classification() 