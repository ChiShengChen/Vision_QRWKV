...
def run_classical_breastmnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical BreastMNIST classification test") # Updated print
    ...
    print(f"Classical Model Config for BreastMNIST: {config}\n")

    model = RWKVModel(config)
    model.to(device)
    print(f"Classical RWKVModel for BreastMNIST instantiated successfully.\n")
    ...
    print(f"Starting classical training for BreastMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Classical BreastMNIST)')
    ...
    print(f"Classical BreastMNIST training finished.\n")
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Classical BreastMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='orange'); plt.title('Test Acc (Classical BreastMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Classical BreastMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_classical_breastmnist_classification() 