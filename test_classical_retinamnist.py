...
# MedMNIST import
from medmnist import RetinaMNIST # Changed from PathMNIST
...
# Function name changed for RetinaMNIST
def run_classical_retinamnist_classification():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Updated print statement for RetinaMNIST
    print(f"Using device: {device} for classical RetinaMNIST classification test")

    training_start_time_obj = datetime.datetime.now()
    training_start_time_str = training_start_time_obj.strftime("%Y%m%d_%H%M%S")
    
    # RetinaMNIST Data loading and preprocessing
    image_size = 8
    batch_size_train = 64
    batch_size_test = 1000
    
    # Specifics for RetinaMNIST
    dataset_name_lower = "retinamnist"
    input_channels = 3 
    num_classes = 5
    # Normalization for RetinaMNIST (RGB)
    mean_val = (0.3698, 0.2242, 0.1048)
    std_val = (0.2991, 0.1930, 0.1149)
    ...
    # Updated base directory name for RetinaMNIST
    ...
    # RetinaMNIST DataLoaders
    # Using as_rgb=True for consistency, though RetinaMNIST is already RGB
    # download=True will download if not found in './data'
    train_dataset = RetinaMNIST(split='train', transform=transform, download=True, as_rgb=True, root='./data')
    test_dataset = RetinaMNIST(split='test', transform=transform, download=True, as_rgb=True, root='./data')
    ...
    # output_dim_test updated to number of classes for RetinaMNIST
    ...
    # Updated print statement for RetinaMNIST config
    print(f"Classical Model Config for RetinaMNIST: {config}\n")
    ...
    print(f"Classical RWKVModel for RetinaMNIST instantiated successfully.\n")
    ...
    # Updated print statement
    print(f"Starting classical training for RetinaMNIST classification...")
    for epoch in range(num_epochs):
        model.train()
        epoch_loss_sum = 0.0
        num_batches_processed = 0
        for batch_idx, (data, target) in enumerate(train_loader):
            # data shape is (batch, C, H, W) e.g. (64, 3, 8, 8)
            # Reshape for RWKV: (batch, seq_len, input_dim) e.g. (64, 64, 3)
            # Permute C to the end, then flatten H, W into seq_len
        ...
        with torch.no_grad():
            for data, target in test_loader:
                ...
                all_preds_epoch.extend(pred.cpu().numpy().flatten()) # .flatten() in case of single element batches
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.figure(figsize=(10, 8)) # Adjusted for 5 classes, can be smaller like (8,6)
            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                        xticklabels=range(num_classes), yticklabels=range(num_classes))
            plt.xlabel('Predicted Label')
            plt.ylabel('True Label')
            # Updated plot title
            plt.title(f'Confusion Matrix - Epoch {epoch+1} (Classical RetinaMNIST)')
    ...
    # Updated print statement
    print(f"Classical RetinaMNIST training finished.\n")
    ...
    if all_epoch_train_losses:
        ...
        plt.ylabel('Loss')
        # Updated plot title
        plt.title('Training Loss Over Epochs (Classical RetinaMNIST)')
        plt.legend()
        plt.grid(True)
    ...
    if all_epoch_test_accuracies:
        ...
        plt.ylabel('Accuracy (%)')
        # Updated plot title
        plt.title('Test Accuracy Over Epochs (Classical RetinaMNIST)')
        plt.legend()
        plt.grid(True)
    ...
    print(f"\n=== Finished Classical RetinaMNIST Classification Test ===\n")

if __name__ == '__main__':
    # Updated function call
    run_classical_retinamnist_classification() 