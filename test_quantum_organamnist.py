...
def run_quantum_organamnist_classification(): # Renamed function
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for quantum OrganAMNIST classification test") # Updated print
    ...
    print(f"Quantum Model Config for OrganAMNIST: {config}\n")

    model = QuantumRWKVModel(config)
    model.to(device)
    print(f"Quantum RWKVModel for OrganAMNIST instantiated successfully.\n")
    ...
    print(f"Starting quantum training for OrganAMNIST classification...")
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % (print_every * 5) == 0 or epoch == num_epochs -1 :
            ...
            plt.figure(figsize=(12, 10)) # Adjusted for 11 classes
            sns.heatmap(cm, annot=True, fmt='d', cmap='Reds', xticklabels=range(num_classes), yticklabels=range(num_classes))
            plt.xlabel('Predicted'); plt.ylabel('True'); plt.title(f'CM Epoch {epoch+1} (Quantum OrganAMNIST)')
    ...
    print(f"Quantum OrganAMNIST training finished.\n")
    ...
    plt.figure(figsize=(12,4)); plt.subplot(1,2,1); plt.plot(all_epoch_train_losses, label='Train Loss'); plt.title('Train Loss (Quantum OrganAMNIST)')
    plt.subplot(1,2,2); plt.plot(all_epoch_test_accuracies, label='Test Acc', color='purple'); plt.title('Test Acc (Quantum OrganAMNIST)'); plt.tight_layout()
    ...
    print(f"\n=== Finished Quantum OrganAMNIST Classification Test ===\n")

if __name__ == '__main__':
    run_quantum_organamnist_classification() 